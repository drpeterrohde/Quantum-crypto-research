...
Zero-entropy random variables have information-theoretic content, reducing the action of the entropy group to the trivial group,
\begin{align}
	\phi:&\, \mathcal{X} \to e,\nonumber\\
...
Different parties in a multi-party communications protocol in general possess different generating sets defining their subjective code groups.

In the context of an encryption protocol the goal is for communicating parties to establish private code groups exposing encrypted data, while ensuring the public code group exposes only entropic objects.
...
Equivalently, treating private key $p$ as a random variable, $g=m\soplus \mathcal{X}_p$ takes the form of a one-time-pad cipher. Where $p$ is an infinite maximum entropy stream we have,
\begin{align}
	H(\mathcal{X}_p) &= 1,\nonumber\\
...
In the zero-entropy case the scheme reduces to a one-time-pad with reused private key, revealing as plaintext parities between messages,
\begin{align}
...
\begin{align}
	...
	\overline{x\soplus y} &= x \oplus \bar{y} = \bar{x} \oplus y.
\end{align}
