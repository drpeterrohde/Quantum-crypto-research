...
\begin{itemize}
	...
	\item Permutations and XOR masks preserve bijectivity. If a function $f: X\to X$ is bijective then so is $f: X\to \pi\circ X \oplus c$. Taking the perfectly symmetric hash model with three orders of braiding (0,1,2) implements the trivial identity map. Adding bijectivity-preserving transformations defines the class of hash models.
	\item The space of all bijective maps over $n$-bit bitstrings has,
		\begin{align}
			&f:\, X\to X,\, |X|=2^n,\nonumber\\
			&|f| = (2^n)!,	
		\end{align}
		assignments. The respective space for the hash model is that afforded by permutation transforming the bijective map,
		\begin{align}
			&|h| = n! = O(\log\log n),
		\end{align}
		[a guess, check that] a reduced subspace of the allowed space of bijective maps. The full space cannot be efficiently encoded. 
		\url{https://chatgpt.com/share/bbe8cbb0-7012-4959-a125-5bacfaac3e07}
		\item The space of unique transformations over $X$ under the action of a permutation group is equivalent to the action of a common bitmask under XOR over the space, an eigenvalue-like relationship,
		\begin{align}
			\pi \circ x &= x \oplus c,\nonumber\\
			S_n \times X &\sim \mathbb{Z}_2^n \times X.
		\end{align}
		\item Becomes,
		\begin{align}
			&f:\, X\to X,\, |X|=2^n,\nonumber\\
...
%			&|h| = 2^{|f|} = O(\log\log n),
%		\end{align}
		\item The action of a permutation group on a space of bit-strings is restricted to that of the action of bit-mask space, but with distinct algebraic properties.
		\item Encoded against maximum entropy all codes becomes NP-hard rather than NP-complete. Enforces brute-force via random sampling. Samples uniformly over all images and pre-images. Not efficiently verifiable. (what does it become?)
		\item Equivalent to \#P since inverse of solution count equates to knowing the respective uniform probability distribution.
		\item Generalising TCFs to $k$ satisfying pre-images, claws correspond to independent points in the automorphism space of the second order braid which originate from distinct automorphism groups over the first braid. This reduces the problem to that of finding two distinct pre-pre-image automorphisms which map to a common pre-image automorphism group.
		\item To construct a generalised TCF we take a single input and calculate
\end{itemize}
...
* Let $\pi\in S_n$ be shared secret key.

* Let user $k$ prepare pair,
\begin{align}
	[\mathcal{X}_i^{(k)}, (\pi\circ \mathcal{X}_i^{(k)}) \oplus \mathcal{X}_j^{(k)}]_k.
\end{align}

* For known $\pi$ the tuple unlocks $\mathcal{X}_j^{(k)}$, a one-time entropy pad.

* For a set of users the established shared secret is,
\begin{align}
	\tilde{\mathcal{X}}_j = \bigoplus_k \mathcal{X}_j^{(k)},
\end{align}
with entropy lower-bounded by the max of each contributing source.

* The first round requires sharing a pair. Subsequent rounds only require sharing a single entropy term where $\mathcal{X}_j$ acts as the implied first term.

* Following the initial two-key round, subsequent rounds daisy chain via differentially braiding against the previous term.

* Affords multi-party unauthenticated secure entropy sharing.
...
\begin{align}
%	A\to B:\, &\{s\soplus \mathcal{X}_i, p \soplus \mathcal{X}_j\},\nonumber\\
%	B\to A:\, &\{s\soplus \mathcal{X}_i', p \soplus \mathcal{X}_j'\}.
	B\to A:\, &\{s\soplus \mathcal{X}_j\}.%, p \soplus m\soplus\mathcal{X}_j\}.
	A\to B:\, &\{s\soplus \mathcal{X}_i^A, \mathcal{X}_i^A \soplus \mathcal{X}_j^A\},\nonumber\\ %p \soplus m\soplus\mathcal{X}_j\},\nonumber\\
	B\to A:\, &\{s\soplus \mathcal{X}_i^B, \mathcal{X}_i^B \soplus \mathcal{X}_j^B\}.%, p \soplus m\soplus\mathcal{X}_j\}.
	A\to B:\quad &\mathtt{pub} = \{s\soplus \mathcal{X}_i, \mathcal{X}_i \soplus \mathcal{X}_j, p \soplus m\soplus\mathcal{X}_j\},\nonumber\\
	B\to A:\quad &\mathtt{pub} = \{s\soplus \mathcal{X}_i, \mathcal{X}_i \soplus \mathcal{X}_j, p \soplus m\soplus\mathcal{X}_j\}.
\end{align}
As $s$ is known to both Alice and Bob the both possess the private code group,
\begin{align}
%	G_\mathrm{C}^{\mathrm{priv}} &= \langle s, p, \mathcal{X}_i, \mathcal{X}_i', \mathcal{X}_j, \mathcal{X}_j'\rangle.
	G &= \{s, \mathcal{X}_i, \mathcal{X}_j, s\soplus \mathcal{X}_i, s\soplus \mathcal{X}_j, \mathcal{X}_i \soplus \mathcal{X}_j, s\soplus \mathcal{X}_i \soplus \mathcal{X}_j\}.\nonumber
\end{align}
The group generated by public information is,
\begin{align}
%	G_\mathrm{C}^{\mathrm{pub}} &= \langle s\soplus \mathcal{X}_i, p \soplus \mathcal{X}_i \soplus \mathcal{X}_j \rangle,\nonumber\\
%	&= \{s\soplus \mathcal{X}_i, p \soplus \mathcal{X}_i \soplus \mathcal{X}_j, s\soplus p \soplus \mathcal{X}_j \}.
	...
	G_\mathrm{C}^{\mathrm{pub}} &= \langle s\soplus \mathcal{X}_i, \mathcal{X}_i \soplus \mathcal{X}_j, p \soplus m\soplus\mathcal{X}_j\rangle,\nonumber\\
	&= \{s\soplus \mathcal{X}_i, s\soplus \mathcal{X}_j, \mathcal{X}_i \soplus \mathcal{X}_j,\nonumber\\
	&\quad p \soplus m\soplus\mathcal{X}_i, p \soplus m\soplus\mathcal{X}_j, \nonumber\\
	&\quad s\soplus p \soplus m\soplus\mathcal{X}_i, s\soplus p \soplus m\soplus\mathcal{X}_j\},
\end{align}
...
A universal gate set may be implemented via the NOT and CNOT (reversible XOR) gates, [complimentary encoding]
\begin{align}
	...
	\nonumber\\
	\mathrm{CNOT}_{i,j}:&\, (b_i b_j,b_j) \to (b_i b_j,b_i),\nonumber\\
\phi:\, &(\mathcal{X}_i \mathcal{X}_j b_i b_j, \mathcal{X}_j b_j) \to (\mathcal{X}_i \mathcal{X}_j b_i b_j, \mathcal{X}_i b_j),\nonumber\\
%	\to &(\mathcal{X}_i\mathcal{X}_i' \mathcal{X}_j\mathcal{X}_j' b_i \soplus b_j, \mathcal{X}_j b_j)
