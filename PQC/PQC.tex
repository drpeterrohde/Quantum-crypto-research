...
Encrypt,
\begin{align}
	\mathtt{Alice}:\, &\stackbraid{h(m)}{m},\nonumber\\
	\mathtt{Bob}:\, &\stackbraid{s}{p},\nonumber\\
	\mathtt{Alice}:\, &h(m),\nonumber\\
	\mathtt{Decrypt}:\, &\stackbraid{h(m)}{m}\oplus \stackbraid{s}{p} = \stackbraid{h(m)\oplus s}{m\oplus p},\nonumber\\
		&\stackbraid{h(m)\oplus s}{m\oplus p} \oplus [h(m)\oplus s]_{\pi^{-1}} \oplus p_\pi\nonumber\\
		&= m_\pi \to m.
\end{align}
...
\begin{align}
	...
	\mathtt{Bob}:\, &\stackbraid{s}{p},\nonumber\\
	\mathtt{Alice}:\, &m,\nonumber\\
	\mathtt{Verify}:\, &\stackbraid{h(m)}{m}\oplus \stackbraid{p}{s} = \stackbraid{h(m)\oplus p}{m\oplus s},\nonumber\\
...
The Hamming weight of a braid is even,
\begin{align}
	\mathrm{wt}\left(\stackbraid{x}{x}\right) \in 2\mathbb{Z},
\end{align}
