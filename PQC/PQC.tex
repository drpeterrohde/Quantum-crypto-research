...
\newcommand{\stackbraid}[2]{{\genfrac{[}{]}{0pt}{}{{#1}}{{#2}}}^{\pi^{-1}}_{\pi}}
\newcommand{\stackbraidpow}[3]{{\genfrac{[}{]}{0pt}{}{{#1}}{{#2}}}^{\pi^{-#3}}_{\pi^#3}}
...
\end{align}
we consider subgroups generated by finite alphabets of letters,
\begin{align}
	\Sigma &= \{g_i \in \{0,1\}^n\}_i,\nonumber\\
...
\begin{align}
	I(\mathcal{X}_i,\mathcal{X}_j) = n\cdot \delta_{i,j},
\end{align}
and for large $n$ are statistically orthogonal, thereby constituting distinct letters in the algebra.
...
The group defined over private group generators is,
\begin{align}
...
\begin{align}
	S_\mathrm{ent} &= \{\mathcal{X}_i,\mathcal{X}_j\},\nonumber\\
	G_\mathrm{ent} &= \langle S_\mathrm{ent} \rangle \nonumber\\
	&= \{\mathcal{X}_i,\mathcal{X}_j, \mathcal{X}_i\soplus \mathcal{X}_j\} \nonumber\\
	&\sim \mathbb{Z}_2 \times \mathbb{Z}_2.
\end{align}

* Zero-entropy random variables are constants with no information-theoretic action, reducing the entropy group to the trivial group,
\begin{align}
	{\tilde S}_\mathrm{ent} &= \{e\},\nonumber\\
	{\tilde G}_\mathrm{ent} &= \langle {\tilde S}_\mathrm{ent}\rangle \sim \mathbf{1}.
\end{align}
...
Zero-entropy defines a group homomorphism mapping the entropy group to the trivial group,
\begin{align}
	\phi: G_\mathrm{pub} \to {\tilde G}_\mathrm{pub},\nonumber\\
	{\tilde G}_\mathrm{ent} = \{e\},\nonumber\\
	\mathrm{ker}({\tilde G}_\mathrm{pub}) = G_\mathrm{ent},\nonumber\\
	{\tilde G}_\mathrm{pub} = G_\mathrm{pub} \,/\, \mathrm{ker}(\phi).
\end{align}
