...
\draw[-{Latex[scale=0.5]}, graphBlueEdgeStyle, line width=2] (node1) -- (node4);
...
\draw[-{Latex[scale=0.5]}, graphBlueEdgeStyle, line width=2] (node1) -- (node7);
\draw[-{Latex[scale=0.5]}, graphRedEdgeStyle, line width=2] (node1) -- (node8);

\node[draw=none] at (node1) {$i$};
\node[draw=none] at (node2) {$j$};
\node[draw=none] at ($(node1)!0.5!(node2) + (0,-0.7)$) {$v_{i,j}=\{0,1\}$};
